...
class RotationalMotor(Motor):
    ...
    def adjustForward(self, debug):
        ...
        if (debug):
            print(
                f"Target: {target % 360} | Current: {currentDeg % 360} Encoder: {self.enc} | Error: {error} | Speed: {control_signal}")
    ...
    def rotate(self, angle, speed, debug):
        ...
        if (abs(error) < 10 and self.enc <= 3):
            ...
            if (debug):
                print(
                    f"Enc: {self.enc} | Error {error} Target: {target} | Current: {current_degrees} | Power: {control_signal}")
            return False
